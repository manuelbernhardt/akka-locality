...
private[locality] final class LocalitySupervisor(settings: LocalitySettings) extends Actor with ActorLogging {
  ...
  def receive: Receive = {
    case m @ MonitorShards(region) =>
      context.child(region.path.name).map { monitor =>
        monitor.forward(m)
      } getOrElse {
...
final case class LocalitySettings(config: Config) {
  ...
  val RetrieveShardStateTimeout: FiniteDuration =
    FiniteDuration(localityConfig.getDuration("retrieve-shard-state-timeout", TimeUnit.MILLISECONDS), TimeUnit.MILLISECONDS)

  val ShardStateUpdateMargin =
    FiniteDuration(localityConfig.getDuration("shard-state-update-margin", TimeUnit.MILLISECONDS), TimeUnit.MILLISECONDS)
