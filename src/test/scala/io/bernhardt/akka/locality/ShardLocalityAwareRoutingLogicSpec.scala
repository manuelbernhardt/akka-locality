  ...
  "The ShardLocalityAwareRoutingLogic" should {
    ...
    "retrieve shard state on startup and use it in order to route messages" in {
      ...
      monitor1 ! ClusterShardingStats(regions = Map(
        region1.ref.path.address -> ShardRegionStats(shards1.map { id => id.toString -> 0 }.toMap),
        region2.ref.path.address -> ShardRegionStats(shards2.map { id => id.toString -> 0 }.toMap),
        region3.ref.path.address -> ShardRegionStats(shards3.map { id => id.toString -> 0 }.toMap)
      ))
